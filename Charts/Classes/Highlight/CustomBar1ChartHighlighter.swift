...
internal class CustomBar1ChartHighlighter: ChartHighlighter
{
    internal init(chart: CustomBar1ChartView)
    {
    ...
    internal override func getHighlight(x x: Double, y: Double) -> ChartHighlight?
    {
        ...
        {
            if let set = _chart?.data?.getDataSetByIndex(h!.dataSetIndex) as? CustomBar1ChartDataSet
            {
                if set.isStacked
                {
    ...
    {
        if let barChartData = _chart?.data as? CustomBar1ChartData
        {
    ...
    /// - parameter yValue:
    /// - returns:
    internal func getStackedHighlight(old old: ChartHighlight?, set: CustomBar1ChartDataSet, xIndex: Int, dataSetIndex: Int, yValue: Double) -> ChartHighlight?
    {
